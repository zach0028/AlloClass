...
async def explain_learning(
    ...
    level_1 = {
        "active": True,
        "message": (
            f"Feedback enregistre : axe '{axis_name}' corrige vers '{cat_name}'."
        ),
    }

    level_2 = {"active": False, "message": "", "proposed_rule": None}
    if config_id and feedback.axis_id:
        from app.services.learning.error_pattern_detector import detect_rule_candidates
    ...
    if config_id:
        from app.services.classification.self_consistency_voting import compute_axis_disagreement
